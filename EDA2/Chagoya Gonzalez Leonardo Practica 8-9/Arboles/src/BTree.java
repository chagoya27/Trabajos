...
public class BTree {
    ...
    public BTree( int m ){ //constructor del arbol
        this.m = m;
        root = new BNode();
        root.setM(m);
    }
    
    public void add( int n ){ //añade el valor en una hoja del arbol en caso de que este valor no exista dentro del arbol
        if( find(n) ){
    ...
    private BNode leafNode( BNode nodo, int n ){ //proceso recursivo que realiza el recorrido para encontrar el nodo hoja mediante la variable n
        if( nodo.leaf ){ //rompe la recursividad
            return nodo;
        }
        else{
            int i = 0;
    ...
    public boolean find(int value){ //determina si el valor n se encuentra dentro del arbol
        if(root.child.isEmpty()==true && root.key.isEmpty()==true){
            return false;
        }
    ...
    //proceso recursivo que va buscando el nodo correspondiente al valor v
    private boolean find(int v,BNode n){
        ...
        if(n.getKey(i)==v){ //si el nodo es encontrado
            return true;
        }
        else{
            if( n.getKey(i) < v)
